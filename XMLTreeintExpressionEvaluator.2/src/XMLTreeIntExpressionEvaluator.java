...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        // Create a concatenation of the str1 and str2 with overlap removed.
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        // Loop through each element of set checking for substring
        boolean found = false;
        ...
        // If string is not a substring, add string to set
        if (!found) {
            strSet.add(str);
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // Loop through each line of file, adding string to set if not substring
        Set<String> set = new Set1L<>();
        while (!(input.atEOS())) {
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int i = 0; i < text.length(); i++) {
            // If charater is '~', print a new line
            if (text.charAt(i) == '~') {
                out.println();
